...
def main():
    # 加载自己的点云文件
    point_cloud_array = np.genfromtxt("/workspace/Pytorch/Dataset/3D-Unet/shapenetcore_partanno_segmentation_benchmark_v0_normal/02691156/1a04e3eab45ca15dd86060f189eb133.txt", delimiter=" ")
    print(point_cloud_array.shape)
    point_cloud_array = DataFrame(point_cloud_array[:, 0:3])  # 选取每一列 的 第0个元素到第二个元素   [0,3) 选取每一行
    # point_cloud_array = point_cloud_array[:, 0:3]  # 选取每一列 的 第0个元素到第二个元素   [0,3) 选取每一行
