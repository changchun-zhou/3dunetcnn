...
def parse_args():
    ...
    parser.add_argument("--name", help="argument defined by user: the name of run case", type=str)
    parser.add_argument("--compress", help="argument defined by user: yaml file",type=str)
    parser.add_argument("--extract", help="argument defined by user: whether extract data", action='store_true')
    parser.add_argument("--device", help="gpu id if defined", type=str)
    parser.add_argument("--resume", help="resume check point", type=str)
    parser.add_argument("--tdvd", help="whether use tdvd", type=str)
    parser.add_argument("--init", help="user config init", type=str)
...
def update_config_to_fit_gpu_memory(config, n_gpus, gpu_memory, output_filename, voxels_per_gb=17000000.0,
    ...
    print("******update_config_to_fit_gpu_memory")
...
def main():
    ...
    print("training_function_kwargs: ", training_function_kwargs)

    # Override the batch size from the config file
    if namespace.batch_size:
        config["batch_size"] = namespace.batch_size
    if namespace.name:
        config["name"] = namespace.name
    if namespace.compress:
        config["compress"] = namespace.compress
    if namespace.extract:
        config["extract"] = namespace.extract
    if namespace.device:
        config["device"] = namespace.device
    if namespace.resume:
        config["resume"] = namespace.resume
    if namespace.tdvd:
        config["tdvd"] = namespace.tdvd
    if namespace.config_filename:
        config['config_filename'] = namespace.config_filename
