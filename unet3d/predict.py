...
def predict_volumetric_batch(model, batch, batch_references, batch_subjects, batch_filenames,
                             ...
                             segmentation_labels, sum_then_threshold, label_hierarchy, write_input_image=False, criterion_name='loss'):
    ...
    for batch_idx in range(len(batch)):
        ...
        '''
        y = batch_references[batch_idx][1]
        prediction = pred_image
        print(y)
        print(np.array(y.get_data()).shape)
        print(prediction)
        print(np.array(prediction.get_data()).shape)
        score = criterion(prediction.get_data().reshape(y.get_data().shape), y.get_data()).item()
        print("score: ", score)
        '''
        ...
        for idx in range(len(dataset)):
            ...
            if len(batch) >= batch_size or idx == (len(dataset) - 1):
                predict_volumetric_batch(model=model, batch=batch, batch_references=batch_references,
                                         ...
                                         write_input_image=write_input_images, criterion_name=criterion_name)
                batch = list()
                batch_references = list()
                batch_subjects = list()
                batch_filenames = list()
