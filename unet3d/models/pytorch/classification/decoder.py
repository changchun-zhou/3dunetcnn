...
from .myronenko_th import MyronenkoLayer, MyronenkoResidualBlock
from . import resnet
...
class BasicDecoder(nn.Module):
    ...
    def forward(self, x):
        for conv1, layer in zip(self.conv1s, self.layers):
            x = conv1(x)
            x = nn.functional.upsample(x, scale_factor=self.upsampling_scale, mode=self.upsampling_mode)
            for block in layer:
                x = block(x)
        return x
    ...
    def __init__(self, base_width=32, layer_blocks=None, layer=MyronenkoLayer, block=MyronenkoResidualBlock,
        ...
        for i, n_blocks in enumerate(layer_blocks):
            ...
            if use_transposed_convolutions:
                ...
                self.upsampling_blocks.append(partial(nn.functional.upsample, scale_factor=upsampling_scale,
